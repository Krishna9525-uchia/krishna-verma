                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="group relative bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20">
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          onClick={() => handleCopy(`primary-${i}`, res.value + (res.unit || ''))}
                    ...
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="group relative bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                         <div className="flex justify-between items-start">
                           ...
                           <button
                            onClick={() => handleCopy(`secondary-${i}`, res.value + (res.unit || ''))}
                            className="opacity-0 group-hover:opacity-100 focus:opacity-100 p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600 transition-all text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
