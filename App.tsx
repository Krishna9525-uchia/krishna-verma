          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="group/result relative bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700 transition-colors hover:bg-slate-100 dark:hover:bg-slate-700/50">
                         <div className="flex justify-between items-start mb-1">
                            <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">{res.label}</span>
                            <button
                              onClick={() => handleCopy(`sec-${i}`, String(res.value))}
                              className={`opacity-0 group-hover/result:opacity-100 focus:opacity-100 transition-opacity p-1 rounded-md hover:text-blue-600 dark:hover:text-blue-400 ${copiedId === `sec-${i}` ? 'text-green-500' : 'text-slate-400'}`}
                              aria-label={copiedId === `sec-${i}` ? "Copied!" : "Copy result"}
                            >
                              {copiedId === `sec-${i}` ? <CheckIcon /> : <CopyIcon />}
                            </button>
                         </div>
