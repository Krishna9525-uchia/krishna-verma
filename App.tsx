...
const CalculatorDetail: React.FC = () => {
  ...
  const handleCopy = (label: string, value: any, unit?: string) => {
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 relative group/res">
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          onClick={() => handleCopy(res.label, res.value, res.unit)}
                          className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-colors focus:outline-none focus:ring-2 focus:ring-white/50 opacity-0 group-hover/res:opacity-100 focus:opacity-100"
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700 relative group/res">
                         <div className="flex justify-between items-start">
                           <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">{res.label}</span>
                           <button
                             onClick={() => handleCopy(res.label, res.value, res.unit)}
                             className="p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50 opacity-0 group-hover/res:opacity-100 focus:opacity-100 text-slate-400"
                             aria-label="Copy result"
                             title="Copy to clipboard"
                           >
                             {copiedId === res.label ? <CheckIcon /> : <CopyIcon />}
