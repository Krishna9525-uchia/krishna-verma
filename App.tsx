             ...
             <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
               {calculator.inputs.map(inp => (
                 <div key={inp.name} className={inp.type === 'select' || calculator.inputs.length < 3 ? "col-span-full" : "col-span-1"}>
                   <label htmlFor={`input-${inp.name}`} className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{inp.label}</label>
                   <div className="relative group">
                      {inp.type === 'select' ? (
                        <select 
                          id={`input-${inp.name}`}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="group/result bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 relative overflow-hidden">
                      <div className="flex justify-between items-start relative z-10">
                        <span className="text-blue-100 text-sm font-medium uppercase tracking-wide">{res.label}</span>
                        <button
                          onClick={() => handleCopy(res.label, String(res.value))}
                          className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors md:opacity-0 group-hover/result:opacity-100 focus:opacity-100"
                          aria-label={copiedLabel === res.label ? "Copied!" : `Copy ${res.label}`}
                        >
                      ...
                      {res.details && <div className="mt-3 pt-3 border-t border-white/20 text-blue-50 text-sm relative z-10">{res.details}</div>}
                   </div>
                 ))}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="group/result bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700 relative">
                         <div className="flex justify-between items-start">
                           <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">{res.label}</span>
                           <button
                            onClick={() => handleCopy(res.label, String(res.value))}
                            className="p-1.5 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors md:opacity-0 group-hover/result:opacity-100 focus:opacity-100 text-slate-400"
                            aria-label={copiedLabel === res.label ? "Copied!" : `Copy ${res.label}`}
                           >
                            {copiedLabel === res.label ? <CheckIcon /> : <CopyIcon />}
                           </button>
