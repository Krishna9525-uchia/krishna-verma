                   ...
                   <div key={i} className="group/result bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 relative">
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg transition-all duration-200 ${copiedLabel === res.label ? 'bg-green-500/20 text-green-300' : 'hover:bg-white/10 text-blue-200 opacity-0 group-hover/result:opacity-100 focus:opacity-100'}`}
                    ...
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="group/result bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700 relative">
                         <div className="flex justify-between items-start">
                           ...
                           <button
                             onClick={() => handleCopy(res.value, res.label)}
                             className={`p-1.5 rounded-md transition-all duration-200 ${copiedLabel === res.label ? 'text-green-500' : 'text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 opacity-0 group-hover/result:opacity-100 focus:opacity-100'}`}
                             aria-label={copiedLabel === res.label ? 'Copied!' : `Copy ${res.label}`}
                             title={`Copy ${res.label}`}
                           >
                             {copiedLabel === res.label ? <CheckIcon /> : <CopyIcon />}
