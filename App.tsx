...
const CalculatorDetail: React.FC = () => {
  ...
  const handleCopy = (id: string, value: any) => {
                    ...
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="group relative bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                         <div className="flex justify-between items-start">
                           ...
                           <button
                             onClick={() => handleCopy(res.label + i, res.value)}
                             className="opacity-0 group-hover:opacity-100 focus:opacity-100 p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600 transition-all"
                             aria-label={copiedId === (res.label + i) ? "Copied!" : "Copy result"}
                           >
