...
const Header: React.FC<{ darkMode: boolean, setDarkMode: (v: boolean) => void }> = ({ darkMode, setDarkMode }) => {
  ...
  return (
    <header className={`fixed w-full top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-sm border-b border-slate-200 dark:border-slate-800' : 'bg-transparent'}`}>
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:z-[100] focus:px-4 focus:py-2 focus:bg-blue-600 focus:text-white focus:rounded-b-lg focus:left-1/2 focus:-translate-x-1/2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all"
        onClick={(e) => {
          e.preventDefault();
