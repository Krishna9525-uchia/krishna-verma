          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                   ...
                   return (
                    <div key={i} className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 relative group/result">
                        <div className="flex justify-between items-start">
                          ...
                          <button
                            onClick={() => copyToClipboard(String(res.value), resultId)}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => {
                      const resultId = `secondary-${res.label}-${i}`;
                      const isCopied = copiedId === resultId;
                      return (
                        <div key={i} className="bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700 group/result relative">
                          <div className="flex justify-between items-start">
                            <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">{res.label}</span>
                            <button
                              onClick={() => copyToClipboard(String(res.value), resultId)}
                              className={`p-1 rounded-md transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${isCopied ? 'text-green-600 opacity-100' : 'text-slate-400 hover:text-blue-600 opacity-0 group-hover/result:opacity-100 focus:opacity-100'}`}
                              aria-label={isCopied ? "Copied!" : "Copy result"}
                            >
                              {isCopied ? <CheckIcon /> : <CopyIcon />}
                            </button>
