                    ...
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="group relative bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                         <div className="flex justify-between items-start mb-1">
                            ...
                            <button
                              onClick={() => handleCopy(res.value, `${res.label}-${i}`)}
                              className="opacity-0 group-hover:opacity-100 focus:opacity-100 p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-400 dark:text-slate-500 transition-all"
