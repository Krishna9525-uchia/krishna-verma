...
const CopyIcon = () => <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m-2 4h5m-2-2l2 2-2 2" /></svg>; const CheckIcon = () => <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>;
...
const CalculatorDetail: React.FC = () => {
  ...
  const handleCopy = (txt: string, id: string) => {
    navigator.clipboard.writeText(txt).then(() => setCopiedId(id)).catch(e => console.error(e));
  };
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 relative group/primary">
                      <div className="flex justify-between items-start">
                        <span className="text-blue-100 text-sm font-medium uppercase tracking-wide">{res.label}</span>
                        <button onClick={() => handleCopy(String(res.value) + (res.unit || ''), `primary-${i}`)} className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 active:bg-white/30 transition-colors focus:outline-none focus:ring-2 focus:ring-white/50" aria-label={copiedId === `primary-${i}` ? "Copied!" : "Copy result"} title={copiedId === `primary-${i}` ? "Copied!" : "Copy result"}>{copiedId === `primary-${i}` ? <CheckIcon /> : <CopyIcon />}</button>
                      </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700 relative group/result">
                         <div className="flex justify-between items-start">
                           <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">{res.label}</span>
                           <button onClick={() => handleCopy(String(res.value) + (res.unit || ''), `${res.label}-${i}`)} className="absolute top-3 right-3 p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 transition-all opacity-0 group-hover/result:opacity-100 focus:opacity-100 focus-visible:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500/50" aria-label={copiedId === `${res.label}-${i}` ? "Copied!" : `Copy ${res.label}`} title={copiedId === `${res.label}-${i}` ? "Copied!" : `Copy ${res.label}`}>{copiedId === `${res.label}-${i}` ? <CheckIcon /> : <CopyIcon />}</button>
                         </div>
