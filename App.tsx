          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="group/result relative bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 transition-all duration-300">
                      <div className="flex justify-between items-start">
                        <span className="text-blue-100 text-sm font-medium uppercase tracking-wide">{res.label}</span>
                        <button
                          onClick={() => copyToClipboard(res.value.toString(), res.label)}
                          className="p-2 rounded-lg bg-white/10 hover:bg-white/20 focus:ring-2 focus:ring-white/50 outline-none transition-all opacity-0 group-hover/result:opacity-100 focus:opacity-100"
