...
const CalculatorDetail: React.FC = () => {
  ...
  const handleCopy = (label: string, value: any) => {
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="relative group/result bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20">
                      <div className="flex justify-between items-start">
                        <span className="text-blue-100 text-sm font-medium uppercase tracking-wide">{res.label}</span>
                        <button
                          onClick={() => handleCopy(res.label, res.value)}
                          className="opacity-0 group-hover/result:opacity-100 focus:opacity-100 p-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-all"
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="relative group/result bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                         <div className="flex justify-between items-start">
                            <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">{res.label}</span>
                            <button
                              onClick={() => handleCopy(res.label, res.value)}
                              className="opacity-0 group-hover/result:opacity-100 focus:opacity-100 p-1 rounded-md bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 transition-all text-slate-500 dark:text-slate-400"
                              aria-label={`Copy ${res.label}`}
                            >
                              {copiedLabel === res.label ? <CheckIcon /> : <CopyIcon />}
                            </button>
