...
const CopyIcon = () => <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect width="14" height="14" x="8" y="8" rx="2" ry="2" strokeWidth={2}/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>;
const CheckIcon = () => <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 6 9 17l-5-5"/></svg>;
...
const CalculatorDetail: React.FC = () => {
  ...
  const handleCopy = (value: string | number, id: string) => {
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4">
                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="group/result relative bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20">
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          onClick={() => handleCopy(res.value, `primary-${i}`)}
                          className="md:opacity-0 group-hover/result:opacity-100 focus:opacity-100 transition-opacity p-2 rounded-lg bg-white/10 hover:bg-white/20 text-white"
                          aria-label={copiedId === `primary-${i}` ? "Copied!" : `Copy ${res.label}`}
                        >
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => (
                      <div key={i} className="group/result relative bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                         <div className="flex justify-between items-start">
                           <span className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">{res.label}</span>
                           <button
                            onClick={() => handleCopy(res.value, `secondary-${i}`)}
                            className="md:opacity-0 group-hover/result:opacity-100 focus:opacity-100 transition-opacity p-1.5 rounded-md bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-600 dark:text-slate-200"
                            aria-label={copiedId === `secondary-${i}` ? "Copied!" : `Copy ${res.label}`}
                           >
