                   ...
                   return (
                     <div key={i} className="group/result bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 relative">
                        <div className="flex justify-between items-start">
                          ...
                          <button
                            onClick={() => handleCopy(String(res.value), resultId)}
                            className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-all opacity-0 group-hover/result:opacity-100 focus:opacity-100 outline-none focus:ring-2 focus:ring-white/30"
                            aria-label={`Copy ${res.label}`}
                          >
                            {copiedId === resultId ? <CheckIcon /> : <CopyIcon />}
                          </button>
                        </div>
                      ...
                      return (
                        <div key={i} className="group/result bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700 relative">
                           <div className="flex justify-between items-start">
                             ...
                             <button
                                onClick={() => handleCopy(String(res.value), resultId)}
                                className="p-1.5 rounded-md bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 transition-all opacity-0 group-hover/result:opacity-100 focus:opacity-100 outline-none focus:ring-2 focus:ring-blue-500/20"
                                aria-label={`Copy ${res.label}`}
                             >
                                {copiedId === resultId ? <CheckIcon /> : <CopyIcon />}
                             </button>
