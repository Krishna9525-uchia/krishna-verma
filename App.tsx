                ...
                <div className="relative flex items-center bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                   ...
                   <input 
                    ...
                    {search && (
                      <button
                        onClick={() => setSearch('')}
                        className="pr-6 text-slate-400 hover:text-slate-600 focus:outline-none focus:text-blue-600"
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-slide-up" style={{animationDelay: '0.1s'}}>
             ...
             <div className="p-6 space-y-4" aria-live="polite" aria-atomic="true">
                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => (
                   <div key={i} className="group/res relative bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 transition-all">
                      <div className="flex justify-between items-start">
                        <span className="text-blue-100 text-sm font-medium uppercase tracking-wide">{res.label}</span>
                        <button
                          onClick={() => handleCopy(res.value)}
                          className={`p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-all opacity-0 group-hover/res:opacity-100 focus:opacity-100 focus:ring-2 focus:ring-white/50 outline-none ${copied ? 'text-green-300' : 'text-white'}`}
  ...
  return (
    <Router>
      ...
      <div className="flex flex-col min-h-screen font-sans">
        <a
          href="#main-content"
          className="fixed -top-16 left-4 z-[100] bg-blue-600 text-white px-4 py-2 rounded-lg transition-all focus:top-4 outline-none ring-2 ring-blue-400"
          onClick={(e) => {
            e.preventDefault();
