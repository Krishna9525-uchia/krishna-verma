...
const Header: React.FC<{ darkMode: boolean, setDarkMode: (v: boolean) => void }> = ({ darkMode, setDarkMode }) => {
  ...
  return (
    <header className={`fixed w-full top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-sm border-b border-slate-200 dark:border-slate-800' : 'bg-transparent'}`}>
      ...
      {/* Mobile Menu */}
      <div id="mobile-menu" className={`md:hidden absolute w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 transition-all duration-300 ease-in-out origin-top ${mobileMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
...
const CalculatorDetail: React.FC = () => {
  ...
  const handleCopy = (value: any, resId: string) => {
             ...
             <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
               {calculator.inputs.map(inp => (
                 <div key={inp.name} className={inp.type === 'select' || calculator.inputs.length < 3 ? "col-span-full" : "col-span-1"}>
                   <label htmlFor={`${idBase}-${inp.name}`} className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{inp.label}</label>
                   <div className="relative group">
                      {inp.type === 'select' ? (
                        <select 
                          id={`${idBase}-${inp.name}`}
                   ...
                   return (
                     <div key={i} className="group/res relative bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20">
                        <div className="flex justify-between items-start">
                          ...
                          <button
                            onClick={() => handleCopy(res.value, resId)}
                            className="opacity-0 group-hover/res:opacity-100 p-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-all focus:opacity-100 outline-none focus:ring-2 focus:ring-white/50"
                            aria-label="Copy result"
                            title="Copy to clipboard"
                          >
                            {copiedId === resId ? <CheckIcon /> : <CopyIcon />}
                          </button>
                        </div>
                      ...
                      return (
                        <div key={i} className="group/res relative bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                           <div className="flex justify-between items-start">
                             ...
                             <button
                                onClick={() => handleCopy(res.value, resId)}
                                className="opacity-0 group-hover/res:opacity-100 p-1 rounded-md bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 transition-all focus:opacity-100 outline-none focus:ring-2 focus:ring-blue-500/50"
                                aria-label="Copy result"
                                title="Copy to clipboard"
                              >
                                {copiedId === resId ? <CheckIcon /> : <CopyIcon />}
