                 ...
                 {results.filter(r => r.isPrimary).map((res, i) => {
                   ...
                   <div key={i} className="group/res relative bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20">
                      <button
                        ...
                        className="absolute top-4 right-4 p-2 rounded-lg bg-white/10 hover:bg-white/20 opacity-0 group-hover/res:opacity-100 transition-all focus:opacity-100 outline-none focus:ring-2 focus:ring-white/50"
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {results.filter(r => !r.isPrimary).map((res, i) => {
                      ...
                      <div key={i} className="group/res relative bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
                         <button
                            onClick={() => handleCopy(res.value, resId)}
                            className="absolute top-2 right-2 p-1.5 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-400 opacity-0 group-hover/res:opacity-100 transition-all focus:opacity-100 outline-none focus:ring-2 focus:ring-blue-500/50"
                            title="Copy to clipboard"
                            aria-label="Copy result"
                          >
                            {copiedId === resId ? <CheckIcon /> : <CopyIcon />}
